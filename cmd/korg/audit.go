...
from (
  ...
  group by
	...
	resp, err := http.Post("https://certmanager.devstats.cncf.io/api/ds/query", "application/json", bytes.NewBuffer(requestBody))
	if err != nil {
		return nil, err
	}
...
func usernameNotInContributors(contribs map[string]Contribution, username string) bool {
	_, found := contribs[strings.ToLower(username)]
